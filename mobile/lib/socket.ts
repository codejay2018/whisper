...
import { GET_BASE_URL } from './axios';
...
export const useSocketStore = create<SocketState>() ((set, get) => ({
  ...
  connect: (token, queryClient) => {
    ...
    const socket = io(GET_BASE_URL(), {auth: { token } });
    ...
    socket.on('new-message', (message:Message)=>{
      ...
      queryClient.setQueryData<Message[]>(['messages', message.chat], (old)=>{
        ...
        const filtered = old.filter((m)=> !m._id.startsWith('tmep-'));
  ...
  sendMessage: (chatId, text, currentUser) => {
    ...
    // add optmistic message immediately
    ...
    Sentry.logger.info('Message sent successfuly', {chatId, messageLenth:text.length});
